...
export default class FelineGrimaceScale extends PageManager {
    onReady() {
        this.scores = { ears: null, eyes: null, muzzle: null, whiskers: null, head: null };
        this.buildUI();
    ...
    updateScorePanel() {
        ...
        unitIds.forEach(id => {
            ...
            if (el) {
                el.textContent = this.scores[id] !== null ? this.scores[id] : '—';
                el.style.color = this.scores[id] !== null ? 'var(--fgs-green)' : 'var(--fgs-text-muted)';
            }
        });
