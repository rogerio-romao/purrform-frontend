...
export default class Fresh extends PageManager {
    ...
    initReviewsCarousel() {
        const wrap = document.querySelector('.fresh-reviews__track-wrap');
        if (!wrap) return;

        const track = wrap.querySelector('.fresh-reviews__track');
        const slides = Array.from(track.querySelectorAll('.fresh-reviews__slide'));
        const prevBtn = document.querySelector('.fresh-reviews__arrow--prev');
        const nextBtn = document.querySelector('.fresh-reviews__arrow--next');
        const slideCount = slides.length;
        let currentIndex = 0;
        let perView = 1;
